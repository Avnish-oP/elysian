  ...
  return (
    <div className="relative min-h-screen pt-20 bg-black">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        ...
        {/* Events Layout - Organic, Staggered Groups */}
        <div className="flex flex-wrap justify-center gap-x-12 gap-y-16 mb-20">
          {events.map((event, index) => {
            // Group cards in rows of 2-3
            // Pattern: 2, 3, 2, 3, 2, 3...
            const cardsPerRow = [2, 3, 2, 3, 2, 3, 2, 3];
            let currentRow = 0;
            ...
            for (let i = 0; i < index; i++) {
              cardsSoFar++;
              countInCurrentRow++;
              if (countInCurrentRow >= cardsPerRow[currentRow % cardsPerRow.length]) {
                currentRow++;
                countInCurrentRow = 0;
              }
            }
            ...
            cardsSoFar++;
            countInCurrentRow++;
            const positionInRow = countInCurrentRow - 1;
            const isNewRow = positionInRow === 0;

            // Stronger vertical offsets for staggered effect within row
            const verticalOffsets = [
              [0, 60],           // Row with 2 cards - more offset
              [-50, 30, 50],     // Row with 3 cards - varied heights
              [40, -40],         // Row with 2 cards - opposite directions
              [20, -60, 45],     // Row with 3 cards - dramatic variation
              [-40, 50],         // Row with 2 cards
              [35, -30, 55],     // Row with 3 cards
            ];

            const verticalOffset = verticalOffsets[currentRow % verticalOffsets.length][positionInRow] || 0;
            ...
            return (
              <EventCard
                ...
                style={{
                  ...
                  marginBottom: isNewRow && index > 0 ? '40px' : '0px',
                }}
              />
            );
          })}
...
function EventCard({ event, index, onClick, style }) {
  ...
  // Vertical rectangle size variations for each card (taller than wide)
  const sizeVariants = [
    { width: "12rem", height: "16rem" },   // Vertical medium
    { width: "11rem", height: "18rem" },   // Tall
    { width: "13rem", height: "17rem" },   // Vertical wide
    { width: "12rem", height: "19rem" },   // Very tall
    { width: "13rem", height: "16rem" },   // Balanced vertical
    { width: "11rem", height: "17rem" },   // Compact tall
  ];

  // Clip-path shapes with convex (bulging) sides
  // Using polygon with curved points to create bulging effect
  const shapeVariants = [
    ...
    "polygon(7% 0%, 93% 0%, 100% 10%, 100% 90%, 93% 100%, 7% 100%, 0% 90%, 0% 10%)",
    "polygon(10% 0%, 90% 0%, 100% 15%, 100% 85%, 90% 100%, 10% 100%, 0% 85%, 0% 15%)",
  ];

  const size = sizeVariants[index % sizeVariants.length];
  const clipPath = shapeVariants[index % shapeVariants.length];
  ...
  return (
    ...
    >
      <motion.div
        ...
        className={`relative w-full h-full overflow-visible shadow-2xl ${event.color} flex items-center justify-center p-6`}
        style={{
      ...
      >
        ...
        >
          <h3
            className={`text-2xl lg:text-3xl font-bold ${
              event.textColor || "text-gray-300"
            } drop-shadow-lg mb-2 leading-tight`}
            style={{ fontFamily: "'Cinzel', serif" }}
          >
            {event.name}
          </h3>
          {event.subtitle && (
            <p className={`text-sm ${event.textColor || "text-gray-400"}`}>
              {event.subtitle}
            </p>
          )}
        </motion.div>
        ...
        <motion.div
          ...
          transition={{ duration: 0.3 }}
          className="absolute inset-0 bg-black/70 backdrop-blur-sm flex flex-col items-center justify-center z-20 p-5"
        >
          <p
            className={`text-sm text-center ${
              event.textColor || "text-gray-300"
            } mb-4 leading-relaxed px-3`}
          >
            {event.description}
          </p>
        ...
        {/* Animated border with clip-path */}
        <motion.div
          initial={{ opacity: 0 }}
